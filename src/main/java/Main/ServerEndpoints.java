...
public class ServerEndpoints {
  ...
  public @ResponseBody ResponseEnvelope<UniquePlayerIdentifier> registerPlayer(
    ...
    if (GameStateVerification.finalData.getGameStateId() == null
        || !GameStateVerification.finalData.getGameStateId().getUniqueGameID().equals(gameID)) {
  ...
  public @ResponseBody ResponseEnvelope<HalfMap> registerHalfMap(
      @PathVariable String gameID, @Validated @RequestBody HalfMap halfMap) throws Exception {
  ...
  public @ResponseBody ResponseEnvelope<GameState> requestGameStateRaw(
    ...
    if (GameStateVerification.finalData.getGameStateId() == null
      ...
      LOGGER.log(
          Level.INFO,
          gameID
              + "    Game does not exist, lookup:  "
              + GameStateVerification.finalData.getGameStateId().getUniqueGameID());
      throw new GenericExampleException("Game not found", "This game does not exist!");
    }
    ...
    LOGGER.log(
        ...
        "player: "
            ...
            + " game: "
            + gameID
            + "    output of game state  "
            + gameStateEnv.getData()
            + "  "
            + gameStateEnv.getState());
  ...
  public @ResponseBody ResponseEnvelope<?> handleException(
      GenericExampleException ex, HttpServletResponse response) {
